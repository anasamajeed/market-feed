...
# ==========================================
# USER CONFIGURATION: TOGGLE EVENT CATEGORIES
# (Set to True to show, False to hide)
# ==========================================
CONFIG = {
    "ENABLE_DIVIDEND_BUY_CUTOFFS": True,   # [DIVIDEND] Must buy before 3:30 PM IST
    "ENABLE_DIVIDEND_PAYOUTS": True,       # [PAYOUT] Demat bank account credit
    "ENABLE_BONUS_AND_SPLITS": True,       # [SPLIT/BONUS] Corporate restructuring
    "ENABLE_EARNINGS_RESULTS": True,       # [RESULTS] Quarterly financial results
    "ENABLE_IPOS_AND_GMP": True,           # [IPO] Open, Close, Allotment & GMP
    "ENABLE_TRADING_HOLIDAYS": True,       # [HOLIDAY] NSE/BSE market closures
    "ENABLE_MACRO_EVENTS": True,           # [MACRO] RBI MPC, US Fed FOMC, CPI
    "ENABLE_FO_EXPIRY": True,              # [F&O] Monthly derivatives expiry
}
...
NSE_TRADING_HOLIDAYS_2026 = {
    ...
    datetime.date(2026, 11, 10): "Diwali Laxmi Pujan (Muhurat Trading Only)",
    datetime.date(2026, 12, 25): "Christmas",
}

# High-impact global & domestic macroeconomic triggers for 2026
MACRO_EVENTS_2026 = [
...
def is_trading_day(d):
    return d.weekday() < 5 and d not in NSE_TRADING_HOLIDAYS_2026
...
def build_pdf_filings_desk(symbol):
    """Direct links to regulatory PDF filings submitted under SEBI LODR."""
    ...
    bse_filings_search = f"https://www.bseindia.com/corporates/ann.html"
    return nse_announcements, bse_filings_search
...
def process_single_ticker(sym, today, cutoff_past, cutoff_future):
    ...
    nse_pdf, bse_pdf = build_pdf_filings_desk(sym)
        ...
        if not divs.empty:
            for ts, amount in divs.items():
                ...
                if cutoff_past <= div_date <= cutoff_future:
                    ...
                    # Cutoff Event
                    if CONFIG.get("ENABLE_DIVIDEND_BUY_CUTOFFS", True):
                        ev_cut = Event()
                        ...
                        ev_cut.add('description', (
                            ...
                            f"• BSE Corporate Filing Desk:\n  {bse_pdf}\n"
                        ))
                        ev_cut.add('location', 'NSE / BSE India')
                        events.append(ev_cut)

                    # Payment Date Event
                    if CONFIG.get("ENABLE_DIVIDEND_PAYOUTS", True):
                        ...
                        ev_pay.add('description', (
                            ...
                            f"• Company: {sym}\n"
                            f"• Declared Amount: ₹{amount:.2f} per share\n"
                            f"• Statutory Mandate: Section 123 Companies Act (within 30 days)\n"
                            f"-----------------------------------------\n"
                            f"• Screener Fundamentals: {screener_link}\n"
                        ))
                        ev_pay.add('location', 'Registered Bank Account')
                        events.append(ev_pay)

        # 2. Stock Splits & Bonus Allotments
        if CONFIG.get("ENABLE_BONUS_AND_SPLITS", True):
            splits = t.splits
            if not splits.empty:
                for ts, ratio in splits.items():
                    ...
                    if cutoff_past <= split_date <= cutoff_future:
                        ...
                        ev_sp.add('description', (
                            ...
                            f"• Screener Fundamentals: {screener_link}\n"
                            f"• Regulatory Disclosures (PDF): {nse_pdf}\n"
                        ))
                        ev_sp.add('location', 'NSE / BSE India')
                        events.append(ev_sp)

        # 3. Financial Results
        if CONFIG.get("ENABLE_EARNINGS_RESULTS", True):
            try:
                cal_df = t.calendar
                            ...
                            if hasattr(ed, "date"):
                                ...
                                if today <= e_date <= cutoff_future:
                                    ...
                                    ev_bm.add('description', (
                                        ...
                                        f"• Outcome PDF Announcements: {nse_pdf}\n"
                                    ))
...
def build_calendar():
    cal = Calendar()
    cal.add('prodid', '-//NSE Nifty 500 Actions, IPOs & Macro Calendar//EN')
    cal.add('version', '2.0')
    cal.add('x-wr-calname', 'NSE Nifty 500 Live Financial Calendar')
    ...
    # 1. Trading Holidays (All-Day Markers)
    if CONFIG.get("ENABLE_TRADING_HOLIDAYS", True):
        for h_date, h_name in NSE_TRADING_HOLIDAYS_2026.items():
            if cutoff_past <= h_date <= cutoff_future:
                ev_h = Event()
                ev_h.add('uid', str(uuid.uuid4()))
                ...
                ev_h.add('location', 'NSE / BSE India')
                cal.add_component(ev_h)
                total_events += 1

    # 2. Ingest Nifty 500 Stocks
    ...
    # 3. Live IPOs & GMP
    if CONFIG.get("ENABLE_IPOS_AND_GMP", True):
        ...
        for ipo in ipos:
            ...
            ev_o.add('description', (
                ...
                f"• Live GMP & Subscription Status (IPOGyani):\n  https://ipogyani.com/\n\n"
                f"• Detailed Reviews & DRHP/RHP Prospectus:\n  https://www.chittorgarh.com/ipo/ipo_dashboard.asp\n"
            ))
    ...
    # 4. Macro Events
    if CONFIG.get("ENABLE_MACRO_EVENTS", True):
    ...
    if CONFIG.get("ENABLE_FO_EXPIRY", True):
        for m in range(4):
            ...
            while last_d.weekday() != 3 or last_d in NSE_TRADING_HOLIDAYS_2026:
